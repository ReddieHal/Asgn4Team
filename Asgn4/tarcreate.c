...
int add_file_rec(int file, char *path)
{
    ...
    if (lstat(path, inode) < 0)
        return -1; // error
    if ((int file_type = get_file_type(inode)) < 0)
        return -1;
...
char *create_header(char *path)
{
    ...
    snprintf(header[263], 6, "00"); // "00"
    if ((struct passwd *u_pwd = getpwuid(inode->st_uid)) != NULL) // user name
        snprintf(header[265], 32, u_pwd->u_name);
    if ((struct group *gr_pwd = getgrgid(inode->st_gid)) != NULL) // group name
        snprintf(header[297], 32, gr_pwd->gr_name);
    ...
    /* STILL NEED TO CALCULATE CHECKSUM */
